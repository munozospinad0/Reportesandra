...
html,body{margin:0;height:100%;background:${NAVY_DK}}
...
@media (max-width:700px){
  ...
  .cover .title{font-size:2.4rem !important}
  .h2{font-size:1.55rem !important;line-height:1.12 !important}
  .tbl{font-size:.74rem !important}
  .tbl th,.tbl td{padding:.4rem .5rem !important}
...
(function(){
  ...
  function bgmPlay(){if(!bgm)return;var p=bgm.play();if(p&&p.catch)p.catch(function(){});var v=bgm.volume;var t=setInterval(function(){v=Math.min(0.10,v+0.012);bgm.volume=v;if(v>=0.10)clearInterval(t);},90);}
  ...
  function playFrom(isAuto){
    ...
    if(src){var a=new Audio(src);audio=a;a.play().then(function(){var total=(isFinite(a.duration)&&a.duration>0)?a.duration*1000:us.length*STEP_MS;a.addEventListener('ended',chainDone,{once:true});begin(total);}).catch(function(){audio=null;if(isAuto){playing=false;auto=false;return;}begin(us.length*STEP_MS);});}
